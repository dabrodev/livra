...
export default function AvatarCreationPage() {
    ...
    const [personaGender, setPersonaGender] = useState<string>("female");
    ...
    // Fetch persona gender and library avatars on mount
    useEffect(() => {
        fetchPersonaGender();
        fetchLibraryAvatars();
    }, [personaId]);

    const fetchPersonaGender = async () => {
        try {
            const response = await fetch(`/api/persona/${personaId}`);
            if (response.ok) {
                const persona = await response.json();
                setPersonaGender(persona.gender || "female");
            }
        } catch (error) {
            console.error("Failed to fetch persona gender:", error);
        }
    };
    ...
    const canGenerate = data.hairColor && data.hairStyle && data.eyeColor && data.skinTone &&
        (personaGender === "male" || data.lipStyle) && // lipStyle only required for females
    ...
    const handleGenerate = async () => {
        ...
        try {
            // Save appearance data to persona first
            await fetch(`/api/persona/${personaId}/appearance`, {
                method: "POST",
            ...
            const response = await fetch(`/api/persona/${personaId}/avatar/generate`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            });
                ...
                <div className="max-w-2xl mx-auto">
                    ...
                    {step === "configure" && (
                        <div className="animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <p className="text-zinc-400 mb-8">Customize your persona&apos;s look</p>
                            ...
                            <div className="space-y-8">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                        {(personaGender === "male" ? maleHairStyles : femaleHairStyles).map((style) => (
                                            <button
                                                key={style.id}
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-3">
                                        {(personaGender === "male" ? maleBodyHeights : femaleBodyHeights).map((height) => (
                                            <button
                                                key={height.id}
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                        {(personaGender === "male" ? maleBodyTypes : femaleBodyTypes).map((type) => (
                                            <button
                                                key={type.id}
                    ...
                    {step === "select" && (
                        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                            ...
                            <p className="text-zinc-400 mb-8 text-center">Select the one that best represents your persona</p>
